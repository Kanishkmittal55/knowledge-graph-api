...
class SettingsMongoDB(BaseModel):
    ...
    def uri(self) -> str:
        ...
        # Check if it's a local MongoDB instance
        
        if self.host in ["localhost", "127.0.0.1"]:
            # Use standard mongodb:// for local instances
            if self.database_name:
                # The key changes: replicaSet=rs0 i.e. --replSet rs0 flag and healthcheck to auto-initialize the replica set. This is not a replica in the traditional sense.
                # ✅ Transactions work (your delete function runs) ✅ Single local database ❌ No actual redundancy/backup ❌ No failover (if container crashes, no automatic recovery)
                return f"mongodb://{self.username}:{password_value}@{self.host}:27017/{self.database_name}?replicaSet=rs0&authSource=admin"
            else:
                return f"mongodb://{self.username}:{password_value}@{self.host}:27017/?replicaSet=rs0&authSource=admin"
        else:
